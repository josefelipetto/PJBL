class SimpsonThreeEigth:
    ...
    def checkInteracoes(self):
        return self.numeroDeIteracoes % 3 == 0
    ...
    def calculate(self):
        ...
        if not self.checkInteracoes():
            print("Número de interações deve ser múltiplo de 3")
            return None
        ...
        for i in range(self.numeroDeIteracoes + 1):
            if i == 0:
                val += ((self.h * 3) / 8) * self.f(self.intervalo[0])
            elif i == self.numeroDeIteracoes:
                val += ((self.h * 3) / 8) * self.f(self.intervalo[1])
            elif i % 3 == 0:
                val += ((self.h * 3) / 8) * (2 * self.f(self.intervalo[0] + (self.h * i)))
            else:
                val += ((self.h * 3) / 8) * (3 * self.f(self.intervalo[0] + (self.h * i)))
